...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.in_planes = NA_C0

        self.conv1  = nn.Conv2d(3, NA_C0, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1    = nn.BatchNorm2d(NA_C0)
        self.layer1 = self._make_layer(block, NA_C0, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, NA_C0*2, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, NA_C0*4, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, NA_C0*8, num_blocks[3], stride=2)
        self.linear = nn.Linear(NA_C0*8*block.expansion, num_classes)
...
class ResNet_cifar(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.layer2 = self._make_layer(block, NA_C0*2, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, NA_C0*4, num_blocks[2], stride=2)
        self.linear = nn.Linear(NA_C0*4*block.expansion, num_classes)
...
class WResNet_cifar(nn.Module):
    def __init__(self, block, num_blocks, k, num_classes=10):
        ...
        self.in_planes = NA_C0*k

        self.conv1 = nn.Conv2d(3, NA_C0*k, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(NA_C0*k)
        self.layer1 = self._make_layer(block, NA_C0*k, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 32*k, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, NA_C0*k, num_blocks[2], stride=2)
        self.linear = nn.Linear(NA_C0*k*block.expansion, num_classes)

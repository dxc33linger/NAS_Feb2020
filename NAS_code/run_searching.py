...
for POP_SIZE in [20]:
    for alpha in [0.2]:
        for mode in ['continual']:
            for DNA_SIZE in [5, 10]:

                if not os.path.exists('../../results/mode_{}'.format(mode)):
                    os.mkdir('../../results/mode_{}'.format(mode))

                command_tmp = 'python search_EA.py  --dataset cifar100 --num_epoch 5 --mode ' + str(mode) + ' --DNA_SIZE '+str(DNA_SIZE) + ' --alpha '+str(alpha) + ' --POP_SIZE ' +str(POP_SIZE)

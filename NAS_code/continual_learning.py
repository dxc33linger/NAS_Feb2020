...
block_cfg = content['block_cfg']
size_cfg = content['size_cfg']
ds_cfg = content['ds_cfg']

block_cfg = [7, 4, 2, 0, 5, 9, 7, 7, 2, 0, 4, 9, 7, 4, 2, 0, 5]
# size_cfg = [16, 16, 256, 32, 512, 16, 16, 16, 16, 256, 24, 16, 16, 16, 256, 64, 64, 16]
for i in size_cfg:
	print(i)

print(block_cfg, size_cfg, ds_cfg)
model = nas.initial_network(block_cfg, size_cfg, ds_cfg)
param = count_parameters_in_MB(nas.net)
method = functions_continual.ContinualNN(model)

model.load_state_dict(torch.load('../../results/model_with_best_fitness'))
...
for epoch in range(args.num_epoch):
	...
	logging.info('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Single-head This training on T0 testing accu is : {:.4f}'.format(nas.test(test_cloud)))
...
torch.save(nas.net.state_dict(), '../../results/model_afterT{}_Accu{:.4f}.pt'.format(0, best_acc_0))
...
for task_id in range(1, total_task):
	...
	for epoch in range(args.epoch_edge):
		train_acc.append(nas.train(epoch, train_edge))
		if epoch % 3 == 0:
			train_acc[-1] = method.train_with_frozen_filter(epoch, train_bm, mask_dict_pre, maskR_dict_pre)
		test_acc_0.append(nas.test(test_cloud))
	...
	torch.save(nas.net.state_dict(), '../../results/model_afterT{}_Accu{:.4f}.pt'.format(task_id, best_acc_mix))
	#
...
scio.savemat('../../results/PSTmain_cfg_{}_acc{:.4f}.mat'.format(block_cfg, best_acc_mix),
			...
			'NA_C0':args.NA_C0, 'epoch': args.num_epoch, 'epoch_edge': args.epoch_edge, 'param':param,
